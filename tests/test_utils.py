...
# ---------------------------------------------------------------------------
# W&B native plot helpers
# ---------------------------------------------------------------------------

class TestWandbPlotHelpers:
    """Tests for log_line_series, log_bar_chart, log_histogram, log_scatter,
    and log_boxplot_as_table — all imported from utils."""

    # Import helpers lazily so the module import doesn't require wandb to exist
    @staticmethod
    def _import_helpers():
        from utils import (
            log_line_series,
        ...
        return log_line_series, log_bar_chart, log_histogram, log_scatter, log_boxplot_as_table

    # ------------------------------------------------------------------
    # No-op when wandb is not installed
    # ------------------------------------------------------------------

    def test_helpers_noop_when_wandb_missing(self, monkeypatch):
        """All helpers must silently do nothing when wandb is not importable."""
        import builtins
        real_import = builtins.__import__

        def _block_wandb(name, *args, **kwargs):
            if name == "wandb":
                raise ImportError("wandb is not installed")
            return real_import(name, *args, **kwargs)

        monkeypatch.setattr(builtins, "__import__", _block_wandb)

        fns = self._import_helpers()
        # Call each helper — they should not raise
        fns[0]("k", [1, 2], [[1.0, 2.0]], ["s"])           # log_line_series
        fns[1]("k", ["a", "b"], [1.0, 2.0])                  # log_bar_chart
        fns[2]("k", {"s": [1.0, 2.0]})                       # log_histogram
        fns[3]("k", [1.0], [2.0])                             # log_scatter
        fns[4]("k", {"g": [1.0, 2.0]})                       # log_boxplot_as_table

    # ------------------------------------------------------------------
    # No-op when wandb.run is None
    # ------------------------------------------------------------------

    def test_helpers_noop_when_no_active_run(self, monkeypatch):
        """Helpers must do nothing when wandb.run is None."""
        try:
            import wandb
        except ImportError:
        ...
        monkeypatch.setattr(wandb, "log", lambda d: logged.append(d))

        fns = self._import_helpers()
        fns[0]("k", [1, 2], [[1.0, 2.0]], ["s"])
        fns[1]("k", ["a"], [1.0])
        fns[2]("k", {"s": [1.0]})
        fns[3]("k", [1.0], [2.0])
        fns[4]("k", {"g": [1.0]})

        assert logged == [], "No wandb.log calls should be made with run=None"

    # ------------------------------------------------------------------
    # log_line_series
    # ------------------------------------------------------------------

    def test_log_line_series_calls_wandb_log(self, monkeypatch):
        try:
            import wandb
        except ImportError:
            pytest.skip("wandb not installed")
        ...
        monkeypatch.setattr(wandb.plot, "line_series",
                            lambda xs, ys, keys, title, xname: f"chart:{title}")
        monkeypatch.setattr(wandb, "log", lambda d: logged.update(d))

        from utils import log_line_series
        log_line_series("my/key", [1, 2, 3], [[0.1, 0.2, 0.3]], ["series1"],
                        title="Test Chart", xname="Layer")

        assert "my/key" in logged

    def test_log_line_series_shared_xs(self, monkeypatch):
        """Shared xs (list[float]) is broadcast to list-of-lists."""
        try:
            import wandb
        except ImportError:
            pytest.skip("wandb not installed")
        ...
        monkeypatch.setattr(wandb, "run", _FakeRun())
        monkeypatch.setattr(wandb, "log", lambda d: received.update(d))
        monkeypatch.setattr(wandb.plot, "line_series",
                            lambda xs, ys, keys, title, xname: xs)

        from utils import log_line_series
        log_line_series("k", [0, 1, 2], [[0.0, 0.1, 0.2], [1.0, 1.1, 1.2]], ["a", "b"])

        # The value should be the xs that was passed to wandb.plot.line_series
        assert received["k"] == [[0, 1, 2], [0, 1, 2]]

    def test_log_line_series_empty_ys_noop(self, monkeypatch):
        try:
            import wandb
        except ImportError:
            pytest.skip("wandb not installed")
    ...
    def test_log_bar_chart_calls_wandb_log(self, monkeypatch):
        try:
            import wandb
        except ImportError:
            pytest.skip("wandb not installed")
        ...
        monkeypatch.setattr(wandb, "run", _FakeRun())
        monkeypatch.setattr(wandb, "Table", lambda columns: type("T", (), {
            "add_data": lambda self, *a: None,
            "columns": columns,
        })())
        monkeypatch.setattr(wandb.plot, "bar",
                            lambda table, lc, vc, title: f"bar:{title}")
        monkeypatch.setattr(wandb, "log", lambda d: logged.update(d))

        from utils import log_bar_chart
        log_bar_chart("bar/key", ["A", "B"], [1.0, 2.0], title="My Bar")

        assert "bar/key" in logged

    def test_log_bar_chart_empty_labels_noop(self, monkeypatch):
        try:
            import wandb
        except ImportError:
            pytest.skip("wandb not installed")
    ...
    def test_log_histogram_uploads_table(self, monkeypatch):
        try:
            import wandb
        except ImportError:
            pytest.skip("wandb not installed")
        ...
        log_histogram("hist/key", {"series_a": [1.0, 2.0], "series_b": [3.0]},
                      title="Distribution")

        assert "hist/key" in logged
        assert len(rows_added) == 3  # 2 + 1 values

    def test_log_histogram_empty_noop(self, monkeypatch):
        try:
            import wandb
        except ImportError:
            pytest.skip("wandb not installed")
    ...
    def test_log_scatter_calls_wandb_log(self, monkeypatch):
        try:
            import wandb
        except ImportError:
            pytest.skip("wandb not installed")
        ...
        monkeypatch.setattr(wandb, "run", _FakeRun())
        monkeypatch.setattr(wandb, "Table", _FakeTable)
        monkeypatch.setattr(wandb.plot, "scatter",
                            lambda table, xl, yl, title: f"scatter:{title}")
        monkeypatch.setattr(wandb, "log", lambda d: logged.update(d))

        from utils import log_scatter
        log_scatter("scat/key", [0.1, 0.5], [0.2, 0.6],
                    x_label="X", y_label="Y", title="Scatter")

        assert "scat/key" in logged

    def test_log_scatter_empty_xs_noop(self, monkeypatch):
        try:
            import wandb
        except ImportError:
            pytest.skip("wandb not installed")
    ...
    def test_log_boxplot_as_table_uploads_rows(self, monkeypatch):
        try:
            import wandb
        except ImportError:
            pytest.skip("wandb not installed")
        ...
        from utils import log_boxplot_as_table
        log_boxplot_as_table(
            "box/key",
            {"qkv": [0.1, 0.2, 0.3], "mlp_expand": [0.4, 0.5]},
            title="Null Space Concentration",
        )

        assert "box/key" in logged
        assert len(rows_added) == 5  # 3 + 2 observations

    def test_log_boxplot_as_table_empty_noop(self, monkeypatch):
        try:
            import wandb
        except ImportError:
            pytest.skip("wandb not installed")
